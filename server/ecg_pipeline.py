"""Trainable ECG decoding pipeline for rhythm and rate classification."""
...
class ECGPipeline:
    """Buffered ECG decoder that classifies several seconds of recent signal."""
    ...
    def on_window(self, window: np.ndarray) -> None:
        ...
        if confidence < 0.45 and "artifact" in config.class_labels:
            class_idx = config.class_labels.index("artifact")
    ...
    def _primary_signal(self, segment: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        arr = segment.astype(np.float64, copy=False)
        leads = arr[: min(3, arr.shape[0])]
        if leads.size == 0:
            leads = arr[:1]
        primary = np.mean(leads, axis=0)
        primary = primary - float(np.mean(primary))
    ...
    def _find_peaks(self, signal: np.ndarray, min_interval_s: float, threshold: Optional[float] = None) -> np.ndarray:
        ...
        if threshold is None:
            threshold = float(np.mean(x) + 0.55 * np.std(x))
    ...
    def _estimate_qrs_widths(self, signal: np.ndarray, peaks: np.ndarray) -> np.ndarray:
        ...
        widths: List[float] = []
        half_window = max(1, int(0.12 * config.sample_rate))
        n = signal.size
        for peak in peaks:
    ...
    def extract_features(self, segment: np.ndarray) -> np.ndarray:
        primary, leads = self._primary_signal(segment)
        span = float(np.max(primary) - np.min(primary))
        rms = float(np.sqrt(np.mean(primary ** 2)))
        diff = np.diff(primary)
        derivative_rms = float(np.sqrt(np.mean(diff ** 2))) if diff.size else 0.0

        threshold = float(np.mean(primary) + 0.55 * np.std(primary))
        peaks = self._find_peaks(primary, min_interval_s=0.35, threshold=threshold)
        ...
        qrs_widths = self._estimate_qrs_widths(primary, peaks)
        qrs_mean = float(np.mean(qrs_widths)) if qrs_widths.size else 0.0
        qrs_std = float(np.std(qrs_widths)) if qrs_widths.size else 0.0

        peak_amps = primary[peaks] if peak_count else np.array([], dtype=np.float64)
        peak_amp_mean = float(np.mean(peak_amps)) if peak_amps.size else 0.0
        peak_amp_std = float(np.std(peak_amps)) if peak_amps.size else 0.0

        lead_rms = np.sqrt(np.mean(leads ** 2, axis=1)) if leads.size else np.zeros(1, dtype=np.float64)
        lead_means = np.mean(leads, axis=1) if leads.size else np.zeros(1, dtype=np.float64)

        corr_values: List[float] = []
        for i in range(leads.shape[0]):
            for j in range(i + 1, leads.shape[0]):
                std_i = float(np.std(leads[i]))
                std_j = float(np.std(leads[j]))
                if std_i > 1e-6 and std_j > 1e-6:
                    corr_values.append(float(np.corrcoef(leads[i], leads[j])[0, 1]))
        mean_corr = float(np.mean(corr_values)) if corr_values else 0.0

        noise_floor = float(np.std(primary - np.convolve(primary, np.ones(5) / 5.0, mode="same")))
    ...
    def start_recording(self, label_name: str) -> None:
        ...
        self._last_record_total = 0
        logger.info("Recording ECG '%s' (label=%s)", label_name, self._current_label)

    def stop_recording(self) -> None:
        self._recording = False
        self._current_label = None
        logger.info("ECG recording stopped. Buffer: %s windows", len(self._train_windows))
    ...
    def clear_training_data(self) -> None:
        ...
        self._is_trained = False
        self._vote_buffer.clear()
        logger.info("ECG training data cleared")
    ...
    def save_model(self, path: str = None) -> str:
        ...
        logger.info("ECG model saved -> %s", path)
        return path
    ...
    def load_model(self, path: str = None) -> bool:
        for meta_path in self._meta_path_candidates(path):
            ...
            try:
                ...
                logger.error("ECG load_model failed: %s", exc)
        return False
